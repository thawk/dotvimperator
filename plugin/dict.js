...
try {
    ...
    var isWindows = config.OS.isWindows;
    var lib = dactyl;
} catch (e) {
    // vimperator
    var opt = options;
    var isWindows = liberator.has("Win32");
    var echomsg = liberator.echomsg;
    var lib = liberator;
}

lib.echomsg(isWindows);
...
/* cache key: zhwiki:resourceloader:filter:minify-css:5:dad3a59b796e0a1722878d4063e1f018 */
		...
		lib.echo(<div id="wikipedia-output">{output}</div>);
	},
...
let dict = {
	...
	init: function(args) {
		...
		let opener = false;
		if (args["-t"])
			opener = {where:lib.CURRENT_TAB};
		if (lib.forceTarget)
			opener = {where:lib.forceTarget};
		if (lib.forceBackground)
			opener = {background:true, where:lib.NEW_TAB};
		if (keyword.length == 0) {
			CommandPromptMode(T(4), update({onSubmit: function(keyword) {
					...
					if (opener)
						return lib.open(dict.engine.href({keyword:decodeURIComponent(dict.keyword), le: args["-l"], type: args["-l"]}), opener);
			...
			)).open(options["dict-clipboard"] ? lib.clipboardRead() : "");
		} else {
			dict.keyword = keyword;
			if (opener)
				return lib.open(dict.engine.href({keyword:decodeURIComponent(dict.keyword), le: args["-l"], type: args["-l"]}), opener);
	...
	clearCache: function(/*args*/) {
		...
		if (word.length == 0) {
			...
			lib.echo("All words has been clear out!");
		} else {
			...
			statement.executeAsync({
					...
					handleCompletion: function(aReason) {
						if (aReason != Components.interfaces.mozIStorageStatementCallback.REASON_FINISHED)
							...
							lib.echo('"' + word + "\" has been removed!");
					}
			});
	...
	process: function(ret) {
		...
		if (ret["notfound"]) {
			lib.echo(T(19) + decodeURIComponent(dict.keyword), commandline.FORCE_SINGLELINE);
			dict.timeout = lib.timeout(ex.redraw, 3000);
		} else {
			...
			switch ( show ) {
				...
				if (dict.args.bang)
					invert = !invert;
				if (invert) {
					lib.echomsg(ret["simple"], 0, commandline.FORCE_SINGLELINE);
					dict.timeout = lib.timeout(ex.redraw, 15000); // TODO: clickable, styling
				} else {
					...
					lib.echo(<>{STYLE}<div class="dict_block" id={"dict_js_"+(dict.args["-e"] || options["dict-engine"] || options.get("dict-engine").defaultValue)}>{list}</div></>, commandline.FORCE_MULTILINE);
					// lib.echomsg(ret["full"]); // commandline.FORCE_MULTILINE
				}
				break;
	...
	google: function(req) {
		if (req.readyState == 4) {
			if (req.status == 200) {
				...
				switch (show) {
					case "s":
					lib.echo(new XML(google.genOutput(g)));
					if (!mow.visible)
						dict.timeout = lib.timeout(ex.redraw, 10000);
					break;
					...
					PopupNotifications.show(gBrowser.selectedBrowser, "dict-popup",
						...
						"dict-popup-anchor", /* anchor ID */
						{
							...
							accessKey: "S",
							callback: function() {
								lib.open("http://translate.google.com/#" + pairs , {background:false, where:lib.NEW_TAB});
							}
						},
					...
					let listener = {
						observe: function(subject, topic, data) {
							if (topic == "alertclickcallback")
								lib.open(data, {background:true, where:lib.NEW_TAB});
						}
					};
					let title = T(34);
	...
	_alert: function(ret) {
		...
		PopupNotifications.show(gBrowser.selectedBrowser, "dict-popup",
			...
			"dict-popup-anchor", /* anchor ID */
			{
				...
				accessKey: "S",
				callback: function() {
					lib.open(dict.engine.href({'keyword':ret['keyword']}), {background:false, where:lib.NEW_TAB});
				}
			},
			null,  /* secondary action */
			{
...
opt.add(["dict-hasaudio", "dich"],
	T(21),
	"boolean",
	true
);

opt.add(["dict-clipboard", "dicb"],
	"clipboard support", // global or selection or both? TODO
	"boolean",
	false
);

opt.add(["dict-simple", "dics"],
	T(22),
	"boolean",
	true
);

opt.add(["dict-engine", "dice"],
	T(23),
	"string",
	"q",
	{
...
opt.add(["dictw-api", "dicwa"],
	"自定义的 mediawiki 系统域名",
	"string",
	"http://zh.wikipedia.org/w/api.php"
);

opt.add(["dict-show", "dico"],
	T(26),
	"string",
	"s",
	{
...
opt.add(["dict-langpair", "dicl"], // stringmap google:en|zh-CN,youdao:jap
	T(17),
	"stringmap",
	"g:zh-CN,y:eng,z:1hp",
	{
...
Array.forEach("dgqyzw", function(char) {
		...
		commands.add(["dict"+char, "di"+char],
	...
	function(args) {
		...
		if (args.bang || !words) {
			...
			if (player && player.getAttribute("src"))
				...
				lib.echo("重新播放失败，无播放器或者播放链接为空！", commandline.FORCE_SINGLELINE);
			return true;
		}
		let le = args["-l"] || "";
		if (le) {
...
// lib.echo(<label class="text-link" xmlns={XUL} href="http://dict.cn/hello" value="hello"/>);
